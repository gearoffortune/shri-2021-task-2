...
export function getDiagramData(
  ...
  polyglot.extend({
    num_commits:
      "%{smart_count} коммит |||| %{smart_count} коммита |||| %{smart_count} коммитов ",
  });
  polyglot.extend({
    num_lines:
      "%{smart_count} строка |||| %{smart_count} строки |||| %{smart_count} строк ",
  });
  const prevCommits = allCommits.filter(
    (entity) =>
      previousSprint.startAt <= entity.timestamp &&
      entity.timestamp <= previousSprint.finishAt
  );
  ...
  const currentByAmount = [
    ...
    currentCommitsWithLOC.filter(
      (commit) => commit.linesOfCode > 500 && commit.linesOfCode <= 1000
    ),
    currentCommitsWithLOC.filter(
      (commit) => commit.linesOfCode > 100 && commit.linesOfCode <= 500
    ),
    currentCommitsWithLOC.filter(
      (commit) => commit.linesOfCode > 0 && commit.linesOfCode <= 100
    ),
  ].map((commits) => commits.reduce((acc, iter) => acc + 1, 0));
  const prevByAmount = [
    prevCommitsWithLOC.filter((commit) => commit.linesOfCode > 1000),
    prevCommitsWithLOC.filter(
      (commit) => commit.linesOfCode > 500 && commit.linesOfCode <= 1000
    ),
    prevCommitsWithLOC.filter(
      (commit) => commit.linesOfCode > 100 && commit.linesOfCode <= 500
