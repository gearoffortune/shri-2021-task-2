...
export function prepareData(
  ...
  const sprints = entities.filter(
    (entity) => entity.type === "Sprint"
  ) as Sprint[];
  const currentSprint = sprints.filter((sprint) => sprint.id === sprintId)[0];
  const previousSprint = sprints.filter(
    (sprint) => sprint.id === sprintId - 1
  )[0];

  const relevantToCurrentSprint = entities.filter((entity) => {
    if (entity.type === "Comment") {
      return (
        currentSprint.startAt <= entity.createdAt &&
        entity.createdAt <= currentSprint.finishAt
      );
    }
    if (entity.type === "Commit") {
      return (
        currentSprint.startAt <= entity.timestamp &&
        entity.timestamp <= currentSprint.finishAt
      );
    }
  });
  const allSummaries = entities.filter(
    (entity) => entity.type === "Summary"
  ) as Summary[];
  const allUsers = entities.filter(
    (entity) => entity.type === "User"
  ) as User[];
  const allCommits = entities.filter(
    (entity) => entity.type === "Commit"
  ) as Commit[];

  const commitsFromCurrentSprint = relevantToCurrentSprint.filter(
    (x) => x.type === "Commit"
  ) as Commit[];
  const leadersData = getLeadersData(
    currentSprint.name,
    allUsers,
    commitsFromCurrentSprint
  );

  const commentsWithLikes = relevantToCurrentSprint.filter(
    (entity) => entity.type === "Comment" && entity.likes.length > 0
  ) as Comment[];
